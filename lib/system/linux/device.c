...
static int metal_vfio_dev_open(struct linux_bus *lbus, struct linux_device *ldev)
{
	...
	strncpy(ldev->dev_name, ldev->dev_name, sizeof(ldev->dev_name)-1);
	ldev->fd               = -1;
	ldev->device.irq_info  = (void *)-1;
	...
	ldev->sdev = sysfs_open_device(lbus->bus_name, ldev->dev_name);
	if (!ldev->sdev) {
		metal_log(METAL_LOG_ERROR, "VFIO: %s:%s not found in sysfs\n",
				lbus->bus_name, ldev->dev_name);
		return -ENODEV;
	}
	metal_log(METAL_LOG_DEBUG, "VFIO: sysfs device opened at %s\n", ldev->sdev->path);

	struct sysfs_attribute *attr = sysfs_get_device_attr(ldev->sdev, "sriov_numvfs");
	if (attr && sysfs_write_attribute(attr, "0", 1) == 0)
		metal_log(METAL_LOG_DEBUG, "VFIO: SR-IOV disabled\n");
	...
	/* Perform device reset with enhanced error handling */
				 ...
				 struct metal_sg *sg_out)
{
	...
	if (!priv || priv->dev_fd < 0) {
		metal_log(METAL_LOG_ERROR, "VFIO: %s - device not open\n", ldev->dev_name);
		return -ENODEV;
	}

	if (nents_in <= 0 || nents_in > 1024) {
		metal_log(METAL_LOG_ERROR, "VFIO: invalid sg list count %d\n", nents_in);
		return -EINVAL;
	}
	...
	/* Check device state again after logging */
	if (!priv || priv->dev_fd < 0) {
		metal_log(METAL_LOG_ERROR, "VFIO: %s - device state changed\n", ldev->dev_name);
		return -ENODEV;
	}

	(void)lbus;

	/* Validate DMA direction flags */
	if (!(dir & (VFIO_DMA_MAP_FLAG_READ | VFIO_DMA_MAP_FLAG_WRITE))) {
		metal_log(METAL_LOG_ERROR, "VFIO: invalid DMA direction flags 0x%x\n", dir);
		return -EINVAL;
	}

	/* Check IOMMU type support */
	if (!ioctl(priv->container_fd, VFIO_CHECK_EXTENSION, VFIO_TYPE1_IOMMU)) {
		metal_log(METAL_LOG_ERROR, "VFIO: TYPE1 IOMMU not supported\n");
		return -EOPNOTSUPP;
	}
                                   ...
                                   int irq)
{
    ...
    if (ioctl(priv->dev_fd, VFIO_DEVICE_GET_IRQ_INFO, &irq_info) < 0) {
        int info_err = errno;
        metal_log(METAL_LOG_ERROR, "VFIO: failed to get IRQ %d info (errno=%d)\n",
                  irq, info_err);
        return;
    }
				...
				struct metal_device **device)
{
	...
	if (strcmp(sdev->driver_name, SYSFS_UNKNOWN) != 0) {
